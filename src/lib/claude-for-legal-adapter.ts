...
export function claudeForLegalSkillPath(command: PatriciaSkillCommand) {
  return `${command.plugin}/skills/${command.slug}/SKILL.md`;
}
...
export async function loadClaudeForLegalSkill(command: PatriciaSkillCommand) {
  ...
  if (!response.ok) {
    const fallback = [`# ${command.command}`, `Source skill missing at ${path}.`, command.promptFrame].join("\n\n");
...
export function buildClaudeForLegalRuntimePrompt(args: {
  ...
  return [
    "Run the workflow from the claude-for-legal SKILL.md source inside Patricia.",
    "The SKILL.md controls the workflow. Do not replace it with old Patricia behavior.",
